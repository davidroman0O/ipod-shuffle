...
describe("Test 'ipodSync' orchestrator", () => {
		...
		await startSync({
			ipodDevicesDb: {
				get: async () => ({
					id: "dev1",
					name: "MyPod",
					lastKnownMountPath: "/Volumes/MyPod",
					playlistIds: ["p1", "p2"],
					groupIds: []
				})
			},
			ipodPlaylistsDb: {
				resolveSource: async (p) => {
					const map = {
						p1: { id: "p1", name: "Rock", trackIds: ["t1", "t2"] },
						p2: { id: "p2", name: "Chill", trackIds: ["t2", "t3"] }
					};
					return map[p.id] || { id: p.id, name: "?", trackIds: [] };
				},
				listByGroup: async () => []
			},
			ipodTracksDb: {
	...
	it("should expand groups into their member playlists", async () => {
		await startSync({
			ipodDevicesDb: {
				get: async () => ({
					id: "dev1", name: "MyPod", lastKnownMountPath: "/v",
					playlistIds: [], groupIds: ["g1"]
				})
			},
			ipodPlaylistsDb: {
				listByGroup: async () => [
					{ id: "g1p1", name: "GroupTrack1", trackIds: [] }
				],
				resolveSource: async (p) => ({ id: p.id, name: "grp", trackIds: ["t1"] })
			},
			ipodTracksDb: {
				resolveByIds: async () => [
		...
		expect(resolved.tracks[0].trackId).toBe("t1");
	});

	it("should resolve aliases to their source playlist's tracks", async () => {
		await startSync({
			ipodDevicesDb: {
				get: async () => ({
					id: "dev1", name: "MyPod", lastKnownMountPath: "/v",
					playlistIds: ["alias1"], groupIds: []
				})
			},
			ipodPlaylistsDb: {
				resolveSource: async () => ({ id: "src1", name: "Source", trackIds: ["t1", "t2"] }),
				listByGroup: async () => []
			},
			ipodTracksDb: {
				resolveByIds: async () => [
					{ id: "t1", sourcePath: "/a.mp3", fileName: "a.mp3", exists: true, sizeBytes: 100 },
					{ id: "t2", sourcePath: "/b.mp3", fileName: "b.mp3", exists: true, sizeBytes: 200 }
				]
			}
		});
		...
		const resolved = await broker.call("ipodSync.resolve", { deviceId: "dev1" });
		expect(resolved.tracks).toHaveLength(2);
		expect(resolved.playlists[0].playlistId).toBe("src1");
	});
	...
	it("should start an async sync and return running immediately", async () => {
		await startSync({
			ipodDevicesDb: {
				get: async () => ({
					id: "dev1", name: "MyPod", lastKnownMountPath: "/Volumes/MyPod",
					playlistIds: ["p1"], groupIds: []
				}),
				recordSync: async (params) => params
			},
			ipodPlaylistsDb: {
				resolveSource: async () => ({ id: "p1", name: "Rock", trackIds: ["t1"] }),
				listByGroup: async () => []
			},
			ipodTracksDb: {
				resolveByIds: async () => [
	...
	it("handleEngineEvent done: persists manifest + marks completed", async () => {
		...
		await startSync({
			ipodDevicesDb: {
				get: async () => ({ id: "dev1", name: "MyPod", lastKnownMountPath: "/v", playlistIds: [], groupIds: [] }),
		...
		await svc.handleEngineEvent(
			{ call: async (n, p) => { if (n === "ipodDevicesDb.recordSync") recorded.push(p); return p; }, emit: async () => {} },
			{ id: "dev1" },
			svc.jobs.get("dev1"),
	...
	it("should throw on unmounted device and not call the engine", async () => {
		await startSync({
			ipodDevicesDb: {
				get: async () => ({ id: "dev1", name: "MyPod", lastKnownMountPath: null, playlistIds: [], groupIds: [] })
			},
			ipodPlaylistsDb: { resolveSource: async () => ({ id: "x", name: "x", trackIds: [] }), listByGroup: async () => [] },
