 ...
 * dependencies (ipodDevicesDb, ipodPlaylists, ipodTracksDb, ipodEngine) are
...
describe("Test 'ipodSync' orchestrator", () => {
	...
	async function startSync(stubs) {
		...
		broker.createService(stubService("ipodPlaylists", stubs.ipodPlaylists || {}));
		...
		broker.createService({
			...
			events: Object.fromEntries(
				eventNames.map(n => [
					n,
					ctx => emittedEvents.push({ name: n, payload: ctx.params })
				])
			)
		});
		await broker.start();
	...
	it("should throw when the device is unknown", async () => {
		...
		await expect(broker.call("ipodSync.resolve", { deviceId: "nope" })).rejects.toBeInstanceOf(
			MoleculerError
		);
	});
	...
	it("handleEngineEvent done: persists manifest + marks completed", async () => {
		...
		await startSync({
			ipodDevicesDb: {
				get: async () => ({
					id: "dev1",
					name: "MyPod",
					lastKnownMountPath: "/v",
					playlistIds: []
				}),
				recordSync: async params => {
		...
		await svc.handleEngineEvent(
			{
				call: async (n, p) => {
					if (n === "ipodDevicesDb.recordSync") recorded.push(p);
					return p;
				},
				emit: async () => {}
			},
			...
			svc.jobs.get("dev1"),
			{
				...
				manifest: [{ trackId: "t1", relativePath: "x", sizeBytes: 100 }],
	...
	it("should throw on unmounted device and not call the engine", async () => {
		...
		await startSync({
			...
			ipodPlaylists: { resolveByIds: async () => [] },
		...
		await expect(broker.call("ipodSync.run", { deviceId: "dev1" })).rejects.toBeInstanceOf(
			MoleculerError
		);
