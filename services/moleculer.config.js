...
module.exports = {
	...
	// This node joins whatever mesh it is deployed into; the namespace is the
	// deployer's concern (env: NAMESPACE), not this component's.
	...
	// Defaults to null (standalone — all services run in-process, no broker
	// needed) so the node works out of the box. To federate into a wider mesh,
	// run a NATS server and set TRANSPORTER=NATS (or a nats:// URL).
	transporter: process.env.TRANSPORTER ? process.env.TRANSPORTER : null,
	...
	metrics: {
		...
		reporter: {
			type: "Prometheus",
			options: {
				// HTTP port — unique to avoid colliding with common dev tools.
				port: process.env.METRICS_PORT || 3380,
				// HTTP URL path
				path: "/metrics",
